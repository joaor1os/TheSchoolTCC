...
include_once '../includes/class/Disciplina.php'; // Inclua a classe Disciplina

$database = new Database();
$db = $database->conn;

$professor = new Professor($db);
$disciplina = new Disciplina($db); // Instanciar a classe Disciplina
$mensagem = '';

// Buscar disciplinas do banco de dados
$disciplinas_result = $disciplina->listarDisciplinas();

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    var_dump($_POST); // Isso vai mostrar o que está sendo enviado pelo formulário
    
    // Limpa o CPF para remover caracteres não numéricos
    $cpf_professor = preg_replace('/\D/', '', $_POST['cpf_professor']);
    
    // Verifica se o CPF tem 11 dígitos
    if (strlen($cpf_professor) !== 11) {
        $mensagem = "CPF deve conter 11 dígitos.";
    } else {
        // Cadastrar novo professor
        $professor->setNomeProfessor($_POST['nome_professor']);
        $professor->setCpfProfessor($cpf_professor); // Usar o CPF limpo
        $professor->setFormacaoProfessor($_POST['formacao_professor']);
        $professor->setDisciplinaProfessor($_POST['disciplina_professor']);
...
<body>
    ...
    <form method="POST" action="cadastrar_professor.php" onsubmit="return validarFormulario();">
        ...
        <label for="disciplina_professor">Disciplina do Docente:</label>
        <select id="disciplina_professor" name="disciplina_professor" required>
            <option value="">Selecione uma disciplina</option>
            <?php while ($row = $disciplinas_result->fetch_assoc()): ?>
                <option value="<?= $row['id_disciplina'] ?>"><?= htmlspecialchars($row['nome_disciplina']) ?></option>
            <?php endwhile; ?>
        </select><br><br>
